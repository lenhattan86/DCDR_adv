...
addpath('matpower4.1');
addpath('matpower4.1/t');
load('traces/testdayirrad.mat');
...
required_length = 5*24*12; % per minute data
...
% lower_bound = ones(T,1)*-DCcapacities;
% upper_bound = ones(T,1)*DCcapacities;
dc_pwr = dc_power/mean(dc_power)*mean(irrad_time/1000*PVcapacity)*dc_ratio;
upper_bound = dc_pwr;
lower_bound = dc_pwr - gen_power_cap(1);
[W, loadLevels] =  comp_vio_wei_bounds(power_case, PVcapacity,...
                Feb26Irrad, minuteloadFeb2012(36001:sampling_interval:36000+T*sampling_interval), ...
                lower_bound, upper_bound, numLoadLevels, ...    
                opt, dcBus, numBuses, pvBus, grid_load_data, 0, conv_power, conv_power_bus, false);
