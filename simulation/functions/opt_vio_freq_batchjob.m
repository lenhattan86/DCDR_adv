...
%     POWER_UNIT = round(POWER_UNIT*SCALE);
    ...
    epsilon = POWER_UNIT/3; % acceptable errors
...
%             P_dc' == sum(loadLevels.*X,1); % use the inequality constraints to improve the speed.
%             P_dc' <= sum(loadLevels.*X,1) + epsilon ;
%             P_dc' >= sum(loadLevels.*X,1) - epsilon ;
            b >= 0;
            if ON_OFF
%                 idle_power = (sum(A_bj.*b, 1) + a')*IP/PP;
                (sum(A_bj.*b, 1) + a')*peak_power_rate == sum(loadLevels.*X,1);
            else    
                sum(A_bj.*b, 1) + a' + IDLE_POWER == sum(loadLevels.*X,1);
            end
%             P_dc(1) <= dc_pwr_cap/ramp_time;
%             P_dc(2:T)-P_dc(1:T-1) <= dc_pwr_cap/ramp_time;
%             P_dc(2:T)-P_dc(1:T-1) >= -dc_pwr_cap/ramp_time;
            sum(b,2) == BS;
            sum(A_bj.*b,2) == BS;
    cvx_end   
